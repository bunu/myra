...
public class FindRulesActivity extends IterativeActivity<Rule> {
	...
	public final static ConfigKey<RuleList> ITERATION_RULELIST = new ConfigKey<>();
	...
	public Rule create() {
		...
		CONFIG.get(ITERATION_RULELIST).add(rule);
	...
	public void update(Archive<Rule> archive) {
		...
		Rule[] rules = CONFIG.get(ITERATION_RULELIST).rules();
	...
	private void niching(Archive<Rule> archive) {
		Rule[] rules = mergeRules(CONFIG.get(ITERATION_RULELIST).rules(), CONFIG.get(CLASSIFIER).rules());
		sort(rules);
	 ...
	 * 
	 * @param iteration
	 *            the rules created in this iteration
	 * @param classifier
	 *            the current best classifier rule collection
	 * @return the merged list of rules
	 */
	private Rule[] mergeRules(Rule[] iteration, Rule[] classifier) {
		Rule[] rules = new Rule[iteration.length + classifier.length];
		int i = 0;
		for (Rule r : iteration) {
			rules[i] = r;
			i++;
		}
